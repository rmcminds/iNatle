...
server <- function(input, output, session) {
  ...
  observe({
    ...
    r$query <- parseQueryString(session$clientData$url_search)
    isolate({
      ...
      if('rarity' %in% names(r$query)) r$rarity <- r$query[['rarity']]
      if('maxchar' %in% names(r$query)) r$maxchar <- r$query[['maxchar']]
    })
    reset_game()
  ...
  observeEvent(input$submit_specific, {
    ...
    if('obs_id' %in% names(r$query)) updateTextInput(session, 'obs_id', value = r$query[['obs_id']])
    if('placename' %in% names(r$query)) r$placename <- r$query[['placename']] else r$placename <- ''
    if('time_choice' %in% names(r$query)) r$time_choice <- r$query[['time_choice']]
    if('input_taxon' %in% names(r$query)) r$input_taxon <- r$query[['input_taxon']] else r$input_taxon <- ''
    if('user_login' %in% names(r$query)) r$user_login <- r$query[['user_login']] else r$user_login <- ''
    if('rarity' %in% names(r$query)) r$rarity <- r$query[['rarity']]
    if('maxchar' %in% names(r$query)) r$maxchar <- r$query[['maxchar']]
  ...
  observeEvent(r$ready, {
    ...
    if(r$ready) {
      ...
      session$sendCustomMessage("update_url", as.list(setNames(URLencode(allparams),names(allparams))))
